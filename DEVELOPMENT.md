...
and word snapshot helpers provide deterministic text representations. Preserve behavior at narrow
dimensions and for both viewport sides. A non-terminal frontend can stop at
`Machine_session`/`Machine_view`, or reuse the application model if its history and viewport
behavior fit.
...
subset: they share a textual ISA, but their instruction encodings and some semantics differ. The
regeneration script is the trust boundary for generated bytes and provenance. Run
`make regenerate-griotte-extracted` to rebuild and atomically install the generated pair from the
configured upstream.
...
and machine behavior. Before committing generated-sensitive work, also confirm that
