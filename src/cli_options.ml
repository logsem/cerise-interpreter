...
let available_backends (() : unit) : string = String.concat ", " (Cerise.Backend_registry.available_backend_names ())
let parse (argv : string array) : (t, string) result =
  let interactive = ref false and backend = ref Cerise.Backend_registry.default_backend_name and mem_size = ref None and regfile = ref None and programs = ref [] in
  ...
  try
    ...
    if Cerise.Backend_registry.find_backend !backend = None then Error (Printf.sprintf "Unknown backend %S. Available backends: %s." !backend (available_backends ())) else
