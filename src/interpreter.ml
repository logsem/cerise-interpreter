(* The executable coordinates file loading, session construction, and selection of
   interactive or batch presentation. Parsing and machine behavior remain in the
   library modules so this entry point only owns process-level diagnostics. *)
...
let print_diagnostics (diagnostics : Diagnostic.t list) : unit =
  List.iter (fun diagnostic -> prerr_endline (Diagnostic.to_string diagnostic)) diagnostics

let exit_with_error (message : string) : 'a =
  prerr_endline message;
  exit 1

let load_optional_regfile (filename : string option) : (string option, string) result =
  match filename with
  | None -> Ok None
  | Some filename -> Result.map Option.some (read_file "Regfile" filename)

let run (arguments : Cli_parser.arguments) : unit =
  match read_file "Program" arguments.program_filename with
  | Error message -> exit_with_error message
  | Ok source -> (
      match load_optional_regfile arguments.regfile_filename with
      | Error message -> exit_with_error message
      | Ok regfile -> (
          match
