...
let () =
  ...
  let regfile_output_name = "asm-toys/bank_loaded.reg" in
  let asm_output_name = "asm-toys/bank_loaded.s" in
  ...
  let (regs, compiled_prog) = Extract.loaded_bank_example Convert.driver
      start_stack end_stack in
  (* let compiled_prog = Extract.linked_bank_example Convert.driver in *)
  let prog =
    List.map
  ...
  List.iter (fun w -> Printf.fprintf oc "%s := %s\n" (pp_regname (fst w))
                (pp_w (Convert.translate_word (snd w)) "(%s, %s, %s, %s, %s)")) regs;
  close_out oc;
