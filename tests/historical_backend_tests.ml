...
let parser_matrix (() : unit) : unit =
  ...
  let typed_macro (locality : string) : string =
    Printf.sprintf
      "%%macro typed(dst: reg, e: expr, v: value, p: perm, l: locality) move $dst $v add $dst $e 1 \
       restrict $dst ($p, $l) # ($p, $l, $e, $e + 8, $e) %%endmacro %%typed(r1, 4, 9, URWL, %s) \
       halt"
      locality
  in
  ...
  let bad_unused =
    [
      ...
      "%macro bad(x: expr) # (RW, $x, 0, 8, 0) %endmacro halt";
    ]
  in
  List.iter
    (fun source ->
      Alcotest.(check bool)
        "u rejects invalid unused typed macro" true
        (Result.is_error (Ucerise.Parser.parse_program source));
      Alcotest.(check bool)
        "m rejects invalid unused typed macro" true
  ...
  List.iter
    (fun op ->
      ...
      Alcotest.(check bool)
        ("m rejects " ^ op) true
        (Result.is_error (Mcerise.Parser.parse_program (op ^ " r1 r2 r3"))))
    removed;
  List.iter
    (fun word ->
      Alcotest.(check bool)
        ("u rejects " ^ word) true
        (Result.is_error (Ucerise.Parser.parse_word word));
      Alcotest.(check bool)
        ("m rejects " ^ word) true
        (Result.is_error (Mcerise.Parser.parse_word word)))
    [ "[SU, GLOBAL, 0, 9, 1]"; "{1: (RW, GLOBAL, 0, 9, 1)}"; "{1: [SU, GLOBAL, 0, 9, 1]}" ];
  List.iter
    (fun value ->
      let source = "move r1 " ^ value in
      Alcotest.(check bool)
        ("u rejects sealing value " ^ value)
        true
        (Result.is_error (Ucerise.Parser.parse_program source));
      Alcotest.(check bool)
        ("m rejects sealing value " ^ value)
        true
        (Result.is_error (Mcerise.Parser.parse_program source)))
    [ "SO"; "S"; "U"; "SU"; "Int"; "Cap"; "SealRange"; "Sealed" ];
  Alcotest.(check bool)
    "u rejects sealing macro kind" true
    (Result.is_error
       (Ucerise.Parser.parse_program "%macro x(p: sealperm) move r1 $p %endmacro %x(S)"));
  Alcotest.(check bool)
    "m rejects sealing macro kind" true
    (Result.is_error
       (Mcerise.Parser.parse_program "%macro x(p: sealperm) move r1 $p %endmacro %x(S)"));
  Alcotest.(check bool)
    "u rejects Directed" true
    (Result.is_error (Ucerise.Parser.parse_word "(URWLX, DIRECTED, 1, 9, 4)"))

let nested_composite_macro_arguments (() : unit) : unit =
  let source =
    "%macro inner(v: value) restrict r1 $v %endmacro %macro outer(p: perm, l: locality) \
     %inner(($p, $l)) %endmacro %outer(URWLX, Local) halt"
  in
  let config = Runtime_config.create ~max_addr:(Z.of_int 64) ~stack_addr:(Z.of_int 32) () in
  let check_u (() : unit) : unit =
    match
      ok (Ucerise.Parser.parse_program source) |> Ucerise.Asm_ir.assemble_program config |> ok
    with
  ...
  let check_m (() : unit) : unit =
    match
      ok (Mcerise.Parser.parse_program source) |> Mcerise.Asm_ir.assemble_program config |> ok
    with
  ...
  check_m ();
  let wrong_kind =
    "%macro inner(v: value) restrict r1 $v %endmacro %macro outer(p: expr, l: locality) \
     %inner(($p, $l)) %endmacro %outer(1, Local) halt"
  in
  Alcotest.(check bool)
...
let generated_construction_and_locations (() : unit) : unit =
  let source =
    "%define N 2 start: move r1 start + N %macro emit(x: expr) # $x %endmacro %emit(N + 5) halt"
  in
  ignore (ok (Ucerise.Parser.parse_program source));
  ignore (ok (Mcerise.Parser.parse_program source));
  ...
  let check_location (type value) (label : string) (expected_line : int) (expected_column : int)
      (matched_value : (value, Diagnostic.t list) result) : unit =
    match matched_value with
    | Error (diagnostic :: _) -> (
        match Diagnostic.location diagnostic with
        | Some location ->
            Alcotest.(check (option string))
              (label ^ " filename") (Some "historical.s") location.source;
  ...
  let u_round_trip =
    ...
    |> Ucerise.Asm_ir.assemble_word config
    |> ok
  in
  ...
  let m_round_trip =
    ...
    |> Mcerise.Asm_ir.assemble_word config
    |> ok
  in
...
let session (backend : string) ?(regfile : string option) (source : string) : Machine_session.t =
  ok (Machine_session.create ~backend ~config ~source ~regfile)

let word (bank : Machine_view.register_bank) (key : string) (s : Machine_session.t) :
    Machine_view.word =
  (Option.get
     (Machine_view.find_register { Machine_view.Register_id.bank; key } (Machine_session.view s)))
    .word

let int_reg (key : string) (s : Machine_session.t) : Z.t =
  Option.get (word Machine_view.General key s).integer

let cap (bank : Machine_view.register_bank) (key : string) (s : Machine_session.t) :
    Machine_view.capability =
  Option.get (word bank key s).capability

let run (s : Machine_session.t) : Machine_session.t = (Machine_session.run s).session

let semantics (() : unit) : unit =
  let u =
    session "ucerise" ~regfile:"r2 := (RW, GLOBAL, 0, 8, 0)" "isptr r1 r2 isptr r3 r4 halt" |> run
  in
  ...
  let u_exec =
    session "ucerise" ~regfile:"pc := (URWX, GLOBAL, 0, 2, 0)" "move r1 17 halt" |> run
  in
  Alcotest.(check string) "u URWX is executable" "17" (Z.to_string (int_reg "r1" u_exec));
  Alcotest.(check bool)
    "u non-executable URW PC fails" true
    ((Machine_session.view
        (session "ucerise" ~regfile:"pc := (URW, GLOBAL, 0, 2, 0)" "move r1 17 halt" |> run))
       .status = Machine_view.Failed);
  let load =
    session "ucerise" ~regfile:"r1 := (URWLX, LOCAL, 0, 8, 3)" "loadu r2 r1 -1 halt # 99" |> run
  in
  Alcotest.(check string) "LoadU below cursor" "99" (Z.to_string (int_reg "r2" load));
  let store =
    session "ucerise" ~regfile:"r1 := (URWLX, LOCAL, 5, 12, 5)" "storeu r1 0 42 halt" |> run
  in
  Alcotest.(check string)
    "StoreU advances cursor" "6"
    (Z.to_string (cap Machine_view.General "r1" store).cursor);
  let promoted =
    session "ucerise" ~regfile:"r1 := (URWLX, LOCAL, 5, 20, 9)" "promoteu r1 halt" |> run
  in
  ...
  let bad = session "ucerise" ~regfile:"r1 := (RW, GLOBAL, 5, 12, 5)" "storeu r1 0 1 halt" |> run in
  Alcotest.(check bool)
    "StoreU permission failure" true
    ((Machine_session.view bad).status = Machine_view.Failed);
  let bad_load =
    session "ucerise" ~regfile:"r1 := (RWLX, LOCAL, 0, 8, 3)" "loadu r2 r1 -1 halt # 99" |> run
  in
  Alcotest.(check bool)
    "LoadU permission failure" true
    ((Machine_session.view bad_load).status = Machine_view.Failed);
  let bad_promote =
    session "ucerise" ~regfile:"r1 := (RWLX, LOCAL, 0, 8, 3)" "promoteu r1 halt" |> run
  in
  Alcotest.(check bool)
    "PromoteU permission failure" true
    ((Machine_session.view bad_promote).status = Machine_view.Failed);
  let local = session "ucerise" "getl r1 stk halt" |> run in
  Alcotest.(check string)
    "u initial Local stack"
    (Z.to_string (Ucerise.Codec.encode_locality Ucerise.Ast.Local))
    (Z.to_string (int_reg "r1" local));
  let localized =
    session "ucerise" ~regfile:"r2 := (URWLX, GLOBAL, 1, 9, 4)" "restrict r2 (URWLX, LOCAL) halt"
    |> run
  in
  Alcotest.(check (option string))
    "u Global can restrict to Local" (Some "LOCAL")
    (cap Machine_view.General "r2" localized).locality;
  let directed = session "mcerise" "getl r1 stk halt" |> run in
  Alcotest.(check string)
    "m initial Directed stack"
    (Z.to_string (Mcerise.Codec.encode_locality Mcerise.Ast.Directed))
    (Z.to_string (int_reg "r1" directed));
  let m_exec =
    session "mcerise" ~regfile:"pc := (URWLX, DIRECTED, 0, 2, 0)" "move r1 19 halt" |> run
  in
  Alcotest.(check string) "m URWLX is executable" "19" (Z.to_string (int_reg "r1" m_exec));
  Alcotest.(check bool)
    "m non-executable URWL PC fails" true
    ((Machine_session.view
        (session "mcerise" ~regfile:"pc := (URWL, DIRECTED, 0, 2, 0)" "move r1 19 halt" |> run))
       .status = Machine_view.Failed);
  let m_u =
    session "mcerise" ~regfile:"r1 := (URWLX, DIRECTED, 0, 8, 5) r3 := (RW, DIRECTED, 0, 8, 0)"
      "loadu r2 r1 -1 promoteu r1 isptr r4 r3 halt # 99"
    |> run
  in
  Alcotest.(check string) "m LoadU" "99" (Z.to_string (int_reg "r2" m_u));
  Alcotest.(check (list string))
    "m PromoteU" [ "RWLX" ] (cap Machine_view.General "r1" m_u).permissions;
  Alcotest.(check string) "m IsPtr" "1" (Z.to_string (int_reg "r4" m_u));
  let directed_store = session "mcerise" "storeu stk 0 42 halt" |> run in
  Alcotest.(check bool)
    "m directed StoreU succeeds" true
  ...
  List.iter
    (fun permission ->
      let stored =
        session "mcerise"
          ~regfile:
            (Printf.sprintf "r1 := (%s, DIRECTED, 0, 20, 10) r2 := (RW, DIRECTED, 0, 8, 5)"
               permission)
          "storeu r1 0 r2 halt"
        |> run
      in
      Alcotest.(check bool)
        ("m " ^ permission ^ " stores fitting Directed capability")
        true
        ((Machine_session.view stored).status = Machine_view.Halted))
    [ "URWL"; "URWLX" ];
  let no_write_local =
    session "mcerise" ~regfile:"r1 := (URW, DIRECTED, 0, 20, 10) r2 := (RW, DIRECTED, 0, 8, 5)"
      "storeu r1 0 r2 halt"
    |> run
  in
  Alcotest.(check bool)
    "m URW rejects fitting Directed capability" true
    ((Machine_session.view no_write_local).status = Machine_view.Failed);
  let bad_directed =
    session "mcerise" ~regfile:"r1 := (URWLX, DIRECTED, 5, 20, 5) r2 := (RW, DIRECTED, 0, 20, 10)"
      "storeu r1 0 r2 halt"
    |> run
  in
  Alcotest.(check bool)
    "directed store read bound" true
...
let sessions_and_edits (() : unit) : unit =
  ...
  in
  Alcotest.(check (list string))
    "word edit" [ "URW" ] (cap Machine_view.General "r2" edited).permissions;
  Alcotest.(check (list string))
    "registry order"
    [
      "vanilla";
