...
let run_prog (filename : string) : mchn =
  ...
  let init_sregs = Machine.init_sreg_state_zeros in
  let init_mems = Machine.init_mem_state Z.(~$0) parse_res in
  let m = Machine.init init_regs init_sregs init_mems in
...
(*       (test_perm (R,W,DL,LM) (get_reg_cap_perm (Reg 1) m null_perm)); *)
...
(*       (test_perm (R,W,DL,LM) (get_reg_cap_perm (Reg 2) m null_perm)); *)
(*   ] *)
...
(*     test_case "deep_ro.s should contain (R-DRO) permission in r1" `Quick *)
(*       (test_perm (R,Ow,LG,DRO) (get_reg_cap_perm (Reg 1) m null_perm)); *)
(*     test_case "deep_ro.s should contain (R-DRO) permission in r2" `Quick *)
(*       (test_perm (R,Ow,LG,DRO) (get_reg_cap_perm (Reg 2) m null_perm)); *)
(*   ] *)
...
let () =
  ...
  run "Run"
    [
      (* ( "Pos", *)
      (*   test_mov_test *)
      (*   @ test_jmper @ test_jmper_jalr @ test_locality_flow @ test_getotype *)
