...
let get_reg_cap_locality (r : regname) (m : Machine.t) (l : locality) =
  match Machine.read_reg r m with Sealable (Cap (_, g, _, _, _)) -> g | _ -> l
...
let test_mov_test =
  ...
  let pc_a =
    match Machine.read_reg PC m with Sealable (Cap (_, _, _, _, a)) -> a | _ -> Z.(~$(-1))
  in
