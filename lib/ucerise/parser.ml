...
let parse_locality = function
  | "GLOBAL" | "Global" -> Some Global | "LOCAL" | "Local" -> Some Local | _ -> None
  ...
  | token :: rest -> (
      ...
      | Identifier name ->
          let parsed =
            ...
            | unsupported ->
                error [token] (Printf.sprintf "Unsupported uCerise instruction `%s`." unsupported)
          in
...
let parse_permission_term = function
  | token :: rest -> (
      ...
      | Identifier name -> (
          ...
          | None -> error [token] "Expected a uCerise permission.")
      | Parameter name -> Ok (Permission_parameter name, rest)
      | _ -> error [token] "Expected a uCerise permission.")
  | [] -> error [] "Expected a uCerise permission."
