          ...
          let result =
            ...
            | "loadu" -> (
                ...
                | Ok (a, rest) -> (
                    ...
                    | Ok (b, rest) ->
                        Result.map
                          (fun (c, rest) -> (LoadU_term (a, b, c), rest))
                          (parse_operand rest)))
            | "storeu" -> ternary_operands (fun a b c -> StoreU_term (a, b, c)) rest
            | "promoteu" -> unary (fun r -> PromoteU_term r) rest
            | "einit" -> binary (fun a b -> EInit_term (a, b)) rest
            | "edeinit" -> unary (fun r -> EDeInit_term r) rest
            | "estoreid" -> binary (fun a b -> EStoreId_term (a, b)) rest
            | "isunique" -> binary (fun a b -> IsUnique_term (a, b)) rest
            | "fail" -> Ok (Fail_term, rest)
            | "halt" -> Ok (Halt_term, rest)
            | unsupported ->
                error [ token ]
                  (Printf.sprintf "Unsupported Cerisier instruction `%s`." unsupported)
          in
          Result.map_error
...
let parse_permission_term = function
  | token :: rest -> (
      ...
      | Identifier name -> (
          ...
          | None -> error [ token ] "Expected a Cerisier permission.")
      | Parameter name -> Ok (Permission_parameter name, rest)
      | _ -> error [ token ] "Expected a Cerisier permission.")
  | [] -> error [] "Expected a Cerisier permission."
...
let reject_inf ?filename source parse =
  match Assembly_frontend.tokenize ?filename source with
  | Ok tokens -> (
      match
        List.find_opt
          (fun token ->
            match Token.kind token with
            | Identifier name -> String.equal (String.lowercase_ascii name) "inf"
            | _ -> false)
          tokens
      with
      | Some token ->
          Error
            [
              Diagnostic.error ~location:(Token.location token)
                "Cerisier bounds are finite; `inf` is not accepted.";
            ]
      | None -> parse ())
  | Error diagnostics -> Error diagnostics

let parse_word ?filename source =
  reject_inf ?filename source (fun () -> Frontend.parse_word ?filename source)

let parse_regfile ?filename source =
  reject_inf ?filename source (fun () -> Frontend.parse_regfile ?filename source)

let parse_program ?filename source =
  reject_inf ?filename source (fun () -> Frontend.parse_program ?filename source)
