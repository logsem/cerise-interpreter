...
let lower_constant config = function
  ...
  | Permission p -> Ok (Mcerise_codec.encode_permission p)
  | Permission_locality (p,l) ->
      let* p = lower_permission p in Result.map (Mcerise_codec.encode_permission_locality p) (lower_locality l)
  | Parameterized_permission_locality (n,_) ->
      diagnostic (Printf.sprintf "Unexpanded permission parameter $%s." n)
  | Locality_constant l -> Ok (Mcerise_codec.encode_locality l)
  ...
  let rec loop addr memory = function
    ...
    | statement::rest -> (
        ...
        | Op term ->
            ...
            match Mcerise_codec.encode op with
...
let rec execute op s =
  ...
  | Restrict (r,o) -> (
      ...
      | Cap (Cap (p,l,b,e,a)),I z -> (
          match Mcerise_codec.decode_permission_locality z with
  ...
  | GetP (r,x) -> (match get x with Cap (Cap (p,_,_,_,_)) ->
      write_next r (I (Mcerise_codec.encode_permission p)) s | _ -> fail s)
  | GetL (r,x) -> (match get x with Cap (Cap (_,l,_,_,_)) ->
      write_next r (I (Mcerise_codec.encode_locality l)) s | _ -> fail s)
...
let step s =
  if s.status <> Running then Error (Machine_backend.Stopped
    ...
    | Cap (Cap (_,_,_,_,a)) -> (
        ...
        | Some (I z) -> (match Mcerise_codec.decode z with Ok op -> Ok (execute op s) | Error _ -> Ok (fail s))
        | _ -> Ok (fail s))
    | _ -> Ok (fail s)
let rec step_n n s =
