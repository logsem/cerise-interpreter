...
let locality (matched_value : locality) : string = match matched_value with Global -> "GLOBAL" | Local -> "LOCAL" | Directed -> "DIRECTED"
...
let operand (matched_value : reg_or_const) : string = match matched_value with Register r -> register r | Const z -> Z.to_string z
