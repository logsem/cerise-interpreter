...
let lower_constant config = function
  ...
  | Permission p -> Ok (Locality_cerise_codec.encode_permission p)
  | Seal_permission p -> Ok (Locality_cerise_codec.encode_seal_permission p)
  | Word_type w -> Ok (Locality_cerise_codec.encode_word_type w)
  | Permission_locality (p, l) ->
      let* p = lower_permission p in
      Result.map (Locality_cerise_codec.encode_permission_locality p) (lower_locality l)
  | Seal_permission_locality (p, l) ->
      let* p = lower_seal_permission p in
      Result.map (Locality_cerise_codec.encode_seal_permission_locality p) (lower_locality l)
  | Parameterized_permission_locality (name, _) ->
      diagnostic (Printf.sprintf "Unexpanded permission parameter $%s." name)
  | Locality_constant l -> Ok (Locality_cerise_codec.encode_locality l)
  ...
  let rec loop address memory = function
    ...
    | statement :: rest -> (
        ...
        | Op term -> (
            ...
            match Locality_cerise_codec.encode op with
...
let rec execute op state =
  ...
  | Restrict (r, o) -> (
      ...
      | Sealable (Cap (p, l, b, e, a)), I z -> (
          match Locality_cerise_codec.decode_permission_locality z with
      ...
      | Sealable (SealRange (p, l, b, e, a)), I z -> (
          match Locality_cerise_codec.decode_seal_permission_locality z with
  ...
  | GetP (r, s) -> (
      ...
      | Sealable (Cap (p, _, _, _, _)) ->
          write_next r (I (Locality_cerise_codec.encode_permission p)) state
      | Sealable (SealRange (p, _, _, _, _)) ->
          write_next r (I (Locality_cerise_codec.encode_seal_permission p)) state
      | _ -> fail state)
  | GetL (r, s) -> (
      match get s state with
      | Sealable sb | Sealed (_, sb) ->
          write_next r (I (Locality_cerise_codec.encode_locality (locality sb))) state
      | _ -> fail state)
  | GetOType (r, s) -> (
      match get s state with
  ...
  | GetWType (r, s) ->
      write_next r (I (Locality_cerise_codec.encode_word_type (word_type (get s state)))) state
  ...
  | Running -> (
      if not (valid_pc state) then Ok (fail state)
        ...
        | Sealable (Cap (_, _, _, _, a)) -> (
            ...
            | Some (I encoded) -> (
                match Locality_cerise_codec.decode encoded with
