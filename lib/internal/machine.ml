...
let init_mem_state (addr_start : Z.t) (prog : t) : mem_state =
  let enc_prog =
    ...
    @@ List.mapi
         ...
         prog
  in
  MemMap.of_seq enc_prog

let get_mem (addr : Z.t) (conf : exec_conf) : word option =
  match MemMap.find_opt addr conf.mem with
  | Some _ as word -> word
  | None -> if Z.(addr >= zero && addr < Parameters.get_max_addr ()) then Some (I Z.zero) else None
  ...
  if is_pc_valid conf then
    ...
    | Some instr -> (
        ...
        | LAnd (r, c1, c2) -> (
            ...
            let w2 = get_word conf c2 in
            match (w1, w2) with
            | I z1, I z2 -> !>(upd_reg r (I Z.(z1 land z2)) conf)
            | _ -> fail_state)
        | LOr (r, c1, c2) -> (
            let w1 = get_word conf c1 in
            let w2 = get_word conf c2 in
            match (w1, w2) with
            | I z1, I z2 -> !>(upd_reg r (I Z.(z1 lor z2)) conf)
            | _ -> fail_state)
        | LShiftL (r, c1, c2) -> (
            let w1 = get_word conf c1 in
            let w2 = get_word conf c2 in
            match (w1, w2) with
            | I z1, I z2 -> !>(upd_reg r (I (lshiftl z1 z2)) conf)
            | _ -> fail_state)
        | LShiftR (r, c1, c2) -> (
            let w1 = get_word conf c1 in
            let w2 = get_word conf c2 in
            match (w1, w2) with
            | I z1, I z2 -> !>(upd_reg r (I (lshiftr z1 z2)) conf)
            | _ -> fail_state)
        | GetL (r1, r2) -> (
            match r2 @! conf with
...
let get_sregfile (m : mchn) : sreg_state = (get_exec_conf m).sreg
let get_memory (m : mchn) : mem_state = (get_exec_conf m).mem
let read_reg (r : regname) (m : mchn) = r @! get_exec_conf m
let read_sreg (r : sregname) (m : mchn) = r @!! get_exec_conf m
