...
let rclear_inv (regs : regname list) : machine_op list = rclear (reg_diff regs)
...
let popU r =
  [ LoadU (r, stk, const (-1)); Sub (Reg 1, const 1, const 1); Lea (stk, Register (Reg 1)) ]
...
let awkward_example radv =
  ...
  @ [
      Store (renv, const 0); (* Local encapsulation *) pushU (Register renv); pushU (Register radv);
    ]
  @ scall_prologue radv []
  @ [ pushU (Register (Reg 0)); Move (Reg 0, const 0); Jmp radv; Lea (stk, const (-6)) ]
  @ popU radv @ popU renv
  @ [ Store (renv, const 1); pushU (Register renv) ]
  @ scall_prologue radv []
  @ [ pushU (Register (Reg 0)); Jmp radv; Lea (stk, const (-6)) ]
  @ popU renv (* Skip the assert *)
  @ rclear_inv [ PC; Reg 0 ]
  @ [ Jmp (Reg 0) ]
