%{
(** Vanilla grammar actions construct only the source forms this backend accepts. The shared lexer
    recognizes a wider cross-backend token vocabulary; recognition alone never grants Vanilla
    syntax. *)
...
let word_type (position : Lexing.position) (name : string) : Ast.word_type =
  ...
  | None -> reject position "Expected a vanilla word type."
%}
