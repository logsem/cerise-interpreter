(** Bidirectional instruction and scalar encodings for the Vanilla backend. Constructor tables are
...
let permission_scalar =
  metadata_scalar ~name:"vanilla permission" [ (O, 0); (E, 1); (RO, 4); (RX, 5); (RW, 6); (RWX, 7) ]
...
let permission_pattern =
  Tagged_metadata_codec.encoding_pattern ~name:"permission" ~tag:0
    ~wrong_tag_error:"not a vanilla permission encoding"
    ~malformed_payload_error:"unknown vanilla permission"
