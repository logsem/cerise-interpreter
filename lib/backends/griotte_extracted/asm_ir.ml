(* This facade owns assembly syntax for the extracted Griotte adapter. Parsed terms
   ...
   undergo concrete assembly into the adapter AST consumed at the handwritten/extracted boundary. *)
...
(* Concrete assembly remains independent from the handwritten backend so the
