(* This facade owns assembly syntax for the extracted Griotte adapter. Parsed terms
   pass through backend-specific macro validation and hygienic substitution, then
   lower to the adapter AST consumed at the handwritten/extracted boundary. *)
...
(* Macro processing: describe extracted-Griotte terms to the shared assembler. *)
...
(* Concrete lowering remains independent from the handwritten backend so the
   extracted machine continues to define its own observable semantics. *)
module Concrete_lowering = struct
  ...
  let lower_regfile (config : Runtime_config.t) (entries : regfile_entry list) :
