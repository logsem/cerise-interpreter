(** Parser entry points translate lexer, grammar, and assembly-pipeline failures into the backend's
    stable diagnostic result type. *)
...
let parse ?(filename : string option)
  ...
  | Generated_parser.Error ->
      diagnostic
        ...
        (Printf.sprintf "Invalid mCerise assembly syntax near %S." (Lexing.lexeme lexbuf))
