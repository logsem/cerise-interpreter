(** Pure mCerise machine transitions. Assembly initializes concrete register and memory maps; each
...
let init (config : Runtime_config.t) (program : word list) (regfile : (register * word) list option)
  ...
  let registers =
    ...
    |> RegMap.add (Reg 31) (Cap (Cap (URWLX, Directed, stack, limit, stack)))
  in
  let registers =
    match regfile with
    | None -> registers
...
let readable_upper_bound (term : word) : Z.t =
  match term with
  | Cap (Cap (p, _, _, e, a)) -> if is_uninitialized_permission p then Z.min a e else e
  | I _ -> Z.zero
...
let locality_flows (requested : locality) (current : locality) : bool =
  ...
  | Directed, _ | Local, Local | Local, Global | Global, Global -> true
  | _ -> false
...
let rec execute (config : Runtime_config.t) (instruction : instruction) (state : t) : t =
  ...
  | Store (r, o) -> (
      ...
      | Cap (Cap (p, _, b, e, a)) when can_write_memory p && b <= a && a < e -> (
          ...
          match w with
          (* Local and directed capabilities need write-local authority. Directed
             capabilities also may not be stored above their readable bound. *)
          | Cap (Cap (_, Local, _, _, _)) when not (has_write_local_permission p) ->
              mark_failed state
          | Cap (Cap (_, Directed, _, _, _))
            when not (has_write_local_permission p && readable_upper_bound w <= a) ->
              mark_failed state
      ...
      | Cap (Cap (p, l, b, e, a)), I off
        when is_uninitialized_permission p && b <= Z.add a off && Z.add a off <= a && a <= e -> (
          ...
          | Cap (Cap (_, Local, _, _, _)) when not (has_write_local_permission p) ->
              mark_failed state
          | Cap (Cap (_, Directed, _, _, _))
            when (not (has_write_local_permission p)) || readable_upper_bound w > Z.add a off ->
              mark_failed state
          | _ ->
              let state =
