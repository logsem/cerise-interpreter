...
let capability
    (p :
      ...
      * Ast.write_permission
      * Ast.deep_local_permission
      * Ast.deep_read_only_permission) (l : Ast.locality) (b : Z.t) (e : Z.t) (a : Z.t) :
    Machine_view.capability option =
  Some
    {
      Machine_view.base = b;
      limit = e;
...
let view_word (word : Ast.word) : Machine_view.word =
  ...
  | Sentry (p, l, b, e, a) ->
      base Sentry None (capability p l b e a) None None [ ("entry", "sentry") ]
  ...
  | Sealed (otype, Cap (p, l, b, e, a)) ->
      base Sealed_capability None (capability p l b e a) None
...
let inspect (state : state) : Machine_view.t =
  ...
  let registers =
    ...
    @ (Machine.SRegMap.bindings state.Machine.system_registers
      |> List.map (fun (system_register, word) ->
          let label = Printer.system_register system_register in
          {
