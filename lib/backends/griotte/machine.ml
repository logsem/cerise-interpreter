...
let shift_right_nonnegative (value : Z.t) (count : Z.t) : Z.t option =
...
let shift_left_z (value : Z.t) (count : Z.t) : Z.t option =
...
let shift_right_z (value : Z.t) (count : Z.t) : Z.t option = shift_left_z value (Z.neg count)
...
let read_memory (config : Runtime_config.t) (address : Z.t) (state : t) : word option =
  match MemMap.find_opt address state.memory with
  | Some word -> Some word
  | None when Z.sign address >= 0 && Z.compare address (Runtime_config.max_addr config) < 0 ->
      Some (I Z.zero)
  | None -> None
...
let init (config : Runtime_config.t) (program : word list)
  ...
  let* registers =
    List.fold_left
      (fun accumulator (r, word) ->
        let* regs = accumulator in
...
let rec execute (config : Runtime_config.t) (instruction : instruction) (state : t) : t =
  ...
  | Jalr (dst, src) -> (
      match register_value PC with
      | Sealable (Cap (p, l, b, e, a)) ->
          set_register PC
            (enter (register_value src))
            (set_register dst (Sentry (p, l, b, e, Z.succ a)) state)
      | _ -> fail state)
  | Jmp offset -> (
      match (operand_value offset, register_value PC) with
  ...
  | Jnz (test, offset) -> (
      ...
      | _ -> execute config (Jmp offset) state)
  ...
  | Restrict (r, o) -> (
      ...
      | _ -> fail state)
  | SubSeg (r, o1, o2) -> (
      match (register_value r, operand_value o1, operand_value o2) with
...
let step (config : Runtime_config.t) (state : t) : (t, Machine_backend.execution_error) result =
  if state.status <> Running then
  ...
  else if not (valid_pc config state) then Ok (fail state)
  else
    match PC @! state with
    | Sealable (Cap (_, _, _, _, address)) -> (
        ...
        | Some (I encoded) -> (
            ...
            | Ok op -> Ok (execute config op state)
