...
let lower_constant config = function
  ...
  | Permission p -> Ok (Vanilla_codec.encode_permission p)
  | Seal_permission p -> Ok (Vanilla_codec.encode_seal_permission p)
  | Word_type w -> Ok (Vanilla_codec.encode_word_type w)
  ...
  let rec loop address memory = function
    ...
    | statement :: rest -> (
        ...
        | Op term -> (
            ...
            match Vanilla_codec.encode op with
...
let rec execute op state =
  ...
  | GetP (r, s) -> (
      match get s state with
      | Sealable (Cap (p, _, _, _)) -> write_next r (I (Vanilla_codec.encode_permission p)) state
      | Sealable (SealRange (p, _, _, _)) ->
          write_next r (I (Vanilla_codec.encode_seal_permission p)) state
      | _ -> fail state)
  | GetOType (r, s) -> (
      match get s state with
  ...
  | GetWType (r, s) ->
      write_next r (I (Vanilla_codec.encode_word_type (word_type (get s state)))) state
  ...
  | Running -> (
      if not (valid_pc state) then Ok (fail state)
        ...
        | Sealable (Cap (_, _, _, a)) -> (
            ...
            | Some (I encoded) -> (
                match Vanilla_codec.decode encoded with
