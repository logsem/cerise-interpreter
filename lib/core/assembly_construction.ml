...
module Expression = struct
  ...
  let rec map_symbol_references (rewrite_symbol : string -> t) (expression : t) : t =
    match expression with
    | Symbol name -> rewrite_symbol name
    | Add (a, b) ->
        Add (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Subtract (a, b) ->
        Subtract (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Multiply (a, b) ->
        Multiply (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Logand (a, b) ->
        Logand (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Logor (a, b) ->
        Logor (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Shift_left (a, b) ->
        Shift_left (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
    | Shift_right (a, b) ->
        Shift_right (map_symbol_references rewrite_symbol a, map_symbol_references rewrite_symbol b)
  ...
  let rec map_parameter_references (substitute_parameter : string -> t option) (expression : t) : t
      =
    match expression with
    | Parameter name as expression -> Option.value (substitute_parameter name) ~default:expression
    | Add (a, b) ->
        Add
          ( map_parameter_references substitute_parameter a,
            map_parameter_references substitute_parameter b )
    | Subtract (a, b) ->
        Subtract
          ( map_parameter_references substitute_parameter a,
            map_parameter_references substitute_parameter b )
  ...
  let rec fold_constant_operations (expression : t) : t =
    match expression with
    | Add (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y -> Integer Z.(x + y)
        | a, b -> Add (a, b))
    | Subtract (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y -> Integer Z.(x - y)
    ...
    | Logand (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y -> Integer (Z.logand x y)
        | a, b -> Logand (a, b))
    | Logor (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y -> Integer (Z.logor x y)
        | a, b -> Logor (a, b))
    | Shift_left (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y when Z.sign y >= 0 && Z.fits_int y ->
            Integer (Z.shift_left x (Z.to_int y))
        | a, b -> Shift_left (a, b))
    | Shift_right (a, b) -> (
        match (fold_constant_operations a, fold_constant_operations b) with
        | Integer x, Integer y when Z.sign y >= 0 && Z.fits_int y ->
            Integer (Z.shift_right x (Z.to_int y))
        | a, b -> Shift_right (a, b))
  ...
  let rec evaluate_with_runtime_config (config : Runtime_config.t) (expression : t) :
      (Z.t, string) result =
    match expression with
    | Integer v -> Ok v
    ...
    | Add (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.map (Z.add x) (evaluate_with_runtime_config config b))
    | Subtract (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.map (Z.sub x) (evaluate_with_runtime_config config b))
    | Multiply (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.map (Z.mul x) (evaluate_with_runtime_config config b))
    | Logand (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.map (Z.logand x) (evaluate_with_runtime_config config b))
    | Logor (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.map (Z.logor x) (evaluate_with_runtime_config config b))
    | Shift_left (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.bind (evaluate_with_runtime_config config b) (fun y ->
                Result.map (Z.shift_left x) (nonnegative_shift_count_as_int y)))
    | Shift_right (a, b) ->
        Result.bind (evaluate_with_runtime_config config a) (fun x ->
            Result.bind (evaluate_with_runtime_config config b) (fun y ->
                Result.map (Z.shift_right x) (nonnegative_shift_count_as_int y)))
    | Current_address -> Error "an unresolved current-address expression remains"
    | Symbol n -> Error (Printf.sprintf "an unresolved symbol %S remains" n)
...
module Make (Syntax : SYNTAX) = struct
  ...
  let diagnostic_at_source_location (location : Diagnostic.source_location) (message : string) :
      Diagnostic.t =
  ...
  let add_source_location_if_missing (location : Diagnostic.source_location)
      (diagnostic : Diagnostic.t) : Diagnostic.t =
  ...
  module Macro_processing = struct
    ...
    let validate_macro_definition
      ...
      let parameters =
        List.map
          (fun (parameter : Syntax.parameter_kind parameter) -> (parameter.name, parameter.kind))
          definition.parameters
      in
      let body_errors =
        List.concat_map
    ...
    let collect_and_validate_macro_definitions
        (items :
          (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, Syntax.parameter_kind) item
          list) :
        ( ( string,
            ( Syntax.statement,
              Syntax.raw_word,
    ...
    let bind_macro_call_arguments
        (definition :
          ( Syntax.statement,
            ...
            Syntax.macro_argument,
            Syntax.parameter_kind )
          macro_definition) (arguments : Syntax.macro_argument list)
        (location : Diagnostic.source_location) :
        ((string * Syntax.macro_argument) list, Diagnostic.t list) result =
    ...
    let expand_macro_calls
        (macro_definitions :
          ( string,
            ...
            macro_definition )
          Hashtbl.t)
        (items : (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, 'a) item list) :
      ...
      in
      let rec expand_item_sequence (expansion_stack : string list)
          (items : (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, 'a) item list) :
          ( (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, 'a) item list,
            Diagnostic.t list )
          result =
        let rec expand_remaining_items
            (expanded_items :
              (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, 'a) item list)
            (remaining_items :
              (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, 'a) item list) :
              ...
              | Macro_call (name, arguments) -> (
                  ...
                  | None ->
                      Error
                        [
                          diagnostic_at_source_location item.location
                            (Printf.sprintf "Unknown macro %S." name);
                        ]
                  | Some definition -> (
                      if List.mem name expansion_stack then
                        ...
                        | Ok bindings -> (
                            ...
                            | Error _ as error -> error
                            | Ok private_labels -> (
                                let renamed_local_labels =
                                  Hashtbl.create (List.length private_labels)
                                in
                                List.iter
                                  (fun label ->
                                    let base =
                                      Printf.sprintf "__macro_%d_%s_%s" !invocation_counter name
                                        label
                                    in
                                    Hashtbl.add renamed_local_labels label
                                      (fresh_macro_local_label base))
                                  private_labels;
                                incr invocation_counter;
                                let rewrite_expression =
                                  rewrite_macro_expression bindings renamed_local_labels
                                in
                                let substitute_macro_body_item
                                    (body_item :
                                      ( Syntax.statement,
                                        Syntax.raw_word,
                                        Syntax.macro_argument,
                                  ...
                                  | Statement statement ->
                                      let statement =
                                        Syntax.map_statement_expressions rewrite_expression
                                          statement
                                      in
                                      Result.map
                                        (fun statement ->
                                  ...
                                  | Macro_call (called, call_arguments) ->
                                      let rec substitute_nested_call_arguments
                                          (transformed : Syntax.macro_argument list)
                                          (remaining_arguments : Syntax.macro_argument list) :
                                          (Syntax.macro_argument list, Diagnostic.t list) result =
                                        match remaining_arguments with
                                        | [] -> Ok (List.rev transformed)
                                        | argument :: rest -> (
                                            let argument =
                                              Syntax.map_argument_expressions rewrite_expression
                                                argument
                                            in
                                            match
                                              Syntax.substitute_argument ~arguments:bindings
                                                argument
                                            with
                                            | Error diagnostics ->
                                                Error
                                                  (List.map
                                                     (add_source_location_if_missing
                                                        body_item.location)
                                                     diagnostics)
                                            | Ok argument ->
                                                substitute_nested_call_arguments
                                                  (argument :: transformed) rest)
                                      in
                                      Result.map
                                        (fun arguments ->
                                ...
                                let rec substitute_macro_body
                                    ...
                                    ( ( Syntax.statement,
                                      ...
                                      Diagnostic.t list )
                                    result =
                                  match remaining_body_items with
                                  | [] ->
                                      expand_item_sequence (name :: expansion_stack)
  ...
  module Symbol_resolution = struct
    ...
    let resolve_symbols_and_remove_declarations
      ...
      Hashtbl.iter
        (fun name (_, location) ->
          if Hashtbl.mem label_addresses name then
            errors :=
              diagnostic_at_source_location location
                (Printf.sprintf "Integer definition %S conflicts with a label of the same name."
                   name)
              :: !errors)
        integer_definitions;
      ...
      let rec resolve_expression_symbols (definitions_being_resolved : string list)
        ...
        | Add (left, right) ->
            Expression.fold_constant_operations
              (Add
                 ( resolve_expression_symbols definitions_being_resolved current_address location
                     left,
                   resolve_expression_symbols definitions_being_resolved current_address location
                     right ))
        | Subtract (left, right) ->
      ...
      let resolved_statements =
        List.filter_map
          (fun item ->
            let resolve_expression = resolve_expression_symbols [] !current_address item.location in
            match item.node with
            | Statement statement ->
  ...
  let assemble_source_program
      (items :
        (Syntax.statement, Syntax.raw_word, Syntax.macro_argument, Syntax.parameter_kind) item list)
      : (Syntax.statement list, Diagnostic.t list) result =
    match Macro_processing.collect_and_validate_macro_definitions items with
    | Error _ as error -> error
    | Ok macros -> (
