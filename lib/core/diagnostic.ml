...
let make ?(severity : severity = Error) ?location:(location : source_location option) (message : string) : t = { severity; location; message }
let error ?location:(location : source_location option) (message : string) : t = make ?location message
let warning ?location:(location : source_location option) (message : string) : t = make ~severity:Warning ?location message
...
let pp (formatter : Format.formatter) (diagnostic : t) : unit = Format.pp_print_string formatter (to_string diagnostic)
