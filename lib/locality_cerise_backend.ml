module Ast = Locality_cerise_ast
module Parser = Locality_cerise_parser
module Printer = Locality_cerise_printer
module Codec = Locality_cerise_codec
module Machine = Locality_cerise_machine
...
let register_of_id (id : Machine_view.Register_id.t) =
  ...
  | General, name when String.length name > 1 && name.[0] = 'r' -> (
      ...
      | _ -> Error [ Diagnostic.error (Printf.sprintf "Unknown locality-Cerise register %S." name) ]
      )
  | _, name ->
