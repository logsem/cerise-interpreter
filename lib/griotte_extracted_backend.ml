module Ast = Griotte_ast
module Parser = Griotte_parser
module Printer = Griotte_printer
module Loader = Griotte_machine
module E = Cerise_internal.Griotte_extracted
module Xcodec = Griotte_extracted_codec
...
let inspect state =
  { (Griotte_backend.inspect state.snapshot) with Machine_view.backend_name = name }
...
let set_register id term state =
  ...
  | _ -> (
      let* register = Griotte_backend.register_of_id id in
